...
func TestSymlinkIfPossible(t *testing.T) {
	...
	workDir, err := TempDir(osFs, "", "fsgoo-symlink")
	if err != nil {
		t.Fatal(err)
	}
	...
	pathFileMem := filepath.Join(memWorkDir, "fsgoom.txt")
	osPath := filepath.Join(workDir, "fsgoo.txt")

	WriteFile(osFs, osPath, []byte("Hi, Fsgo!"), 0777)
	...
	testLink(basePathFs, "fsgoo.txt", "basepath/link.txt", nil)
...
func TestReadlinkIfPossible(t *testing.T) {
	...
	workDir, err := TempDir(osFs, "", "fsgoo-readlink")
	if err != nil {
		t.Fatal(err)
	}
